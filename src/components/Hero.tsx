...
const PortfolioMosaic = ({ data }: Props) => (
  <section className='mt-16 grid grid-cols-1 gap-2.5 overflow-hidden text-white sm:grid-cols-12'>
    <article className='relative min-h-65 overflow-hidden rounded-tl-2xl rounded-tr-[3rem] rounded-br-xl rounded-bl-[3rem] bg-[#119bb6] px-7 py-7 sm:col-span-8'>
      <h2 className='max-w-xl text-2xl font-semibold leading-tight sm:text-3xl'>Currently, I'm building Basobas</h2>
      <p className='mt-4 max-w-xl text-lg leading-6 text-white/95'>
        Basobas is a smart rental management portal that automates monthly rent and utility calculations, then generates
        invoices on schedule.
      </p>
      <p className='mt-3 max-w-xl text-lg leading-6 text-white/95'>
        Landlords can notify tenants instantly by email or WhatsApp, manage everything from a feature-rich dashboard,
        and work with Nepal's Nepali calendar (B.S.)
      </p>
      <a
        href='#projects'
    ...
    <article className='min-h-62.5 rounded-tl-[3rem] rounded-tr-2xl rounded-br-[3rem] rounded-bl-xl bg-[#242424] px-7 py-7 sm:col-span-4 sm:min-h-0'>
      <h2 className='text-2xl font-semibold leading-tight sm:text-3xl'>
        <span className='inline-flex items-center gap-2'>Working at</span>
        <br />
      ...
      <p className='mt-4 text-lg leading-6 text-white/85'>
        Software developer handling backend systems and user experience at Skillprompt.
      </p>
    ...
    {/* <article className='relative min-h-65 overflow-hidden rounded-4xl bg-[#ffb300] sm:col-span-1 sm:min-h-0'>
    ...
    <article className='min-h-62.5 overflow-hidden rounded-4xl bg-[#008c4a] px-7 py-7 sm:col-span-2 sm:min-h-0'>
    ...
    <article className='min-h-57.5 rounded-tl-[3rem] rounded-tr-xl rounded-br-[3rem] rounded-bl-2xl bg-yellow-950 px-7 py-7 sm:col-span-3 sm:row-span-1 sm:min-h-0'>
      <h2 className='text-2xl font-semibold sm:text-3xl'>Find Me Here</h2>
      <div className='mt-7 grid grid-cols-2 overflow-hidden rounded-lg border border-white/35'>
        {SOCIALS.map((social) => (
          <a
            key={social.name}
            href={social.url}
            aria-label={social.name}
            className={`flex h-20 items-center justify-center border-b border-r border-white/25 text-3xl font-bold transition-colors last:border-b-0 nth-[2n]:border-r-0 nth-[n+3]:border-b-0 ${social.color}`}
          >
            {getIcon(social.icon, 36)}
          </a>
        ))}
      </div>
    </article>

    <article className='min-h-57.5 overflow-hidden rounded-tl-xl rounded-tr-[3rem] rounded-br-2xl rounded-bl-[3rem] bg-[#176B4D] px-7 py-7 sm:col-span-9 sm:min-h-0'>
      <h2 className='text-2xl font-semibold sm:text-3xl'>Github Contributions</h2>
      {/* <p className='mt-4 text-sm text-white/90'>A snapshot of my GitHub contributions.</p> */}
    ...
    <section className='flex flex-col justify-center pt-16'>
      <div className='mx-auto w-full max-w-6xl'>
        ...
        <div className='max-w-4xl'>
          ...
          <div className='mt-8 flex flex-wrap items-center gap-3 text-lg text-zinc-200 sm:text-xl'>
            <span>A software engineer and tech enthusiast based in</span>
            <span className='font-bold text-white'>Butwal, Nepal</span>
            <img src='/nepal-flag.gif' alt='Nepal flag' className='h-9 w-12 rounded-sm object-cover' />
          </div>

          <p className='text-lg leading-relaxed text-zinc-200 sm:text-xl'>
            Welcome to my digital world, where I turn ideas into world-class products.
          </p>

          <div className='mt-10 flex flex-wrap gap-4'>
            <a
              href='/blog'
              className='inline-flex items-center justify-center rounded-full border border-[#7ee7c6]/60 bg-linear-to-r from-[#98f1c8] via-[#6fe7c0] to-[#64d09b] px-7 py-3 text-lg font-semibold text-[#07130d] shadow-[0_10px_30px_rgba(110,232,176,0.35)] transition-transform duration-200 hover:scale-[1.02]'
            >
              Read blog
            </a>
            <a
              href='#projects'
              className='inline-flex items-center justify-center rounded-full border border-[#7ee7c6]/40 bg-transparent px-7 py-3 text-lg font-semibold text-white shadow-[inset_0_0_0_1px_rgba(126,231,198,0.25)] transition-colors duration-200 hover:border-[#7ee7c6]/70 hover:bg-[#7ee7c6]/5'
            >
              My journey
            </a>
          </div>
